...
class SOM(object):
    ...
    def generate_network(self, size , citys):
        ...
        network = np.ones((size, 2))
        a = int(size / 4)
        network[0:a, 0] = np.linspace(x_min, x_max, a)
        network[0:a, 1] = y_min
        network[a: 2 * a, 0] = x_max
        network[a: 2 * a, 1] = np.linspace(y_min, y_max, a)
        network[2 * a: 3 * a, 0] = np.linspace(x_max, x_min, a)
        network[2 * a: 3 * a, 1] = y_max
        network[3 * a: 4 * a, 0] = x_min
        network[3 * a: 4 * a, 1] = np.linspace(y_max, y_min, a)
        network = np.row_stack((network, network[0]))  # 末尾添首项，组成闭环
        return network
    ...
    def select_closest(self, network, city):
        ...
        winner_idx = distances.argmin()

        # 计算orc系数,用于神经元权值偏移量，对应论文中的Z
        distence = distances.min()
        #self.orc = np.exp((-(distence ** 2) / (2 * self.radius ** 2)))
    ...
    def som(self):
        ...
        n = citys.shape[0] * 4 #输出层神经元的个数：城市个数的八倍
        network = self.generate_network(n , citys) #随机生成【0，1】分布的神经元的位置
        ...
        for i in range(self.iteraton):
            ...
            network += self.orc * gaussian[:, np.newaxis] * self.learning_rate * (city - network)

            self.learning_rate = self.learning_rate * 0.99997 #学习率的衰减值

            # 更新渗透区域
            self.radius = self.radius_0 * (1 / (1 + np.exp(-i / 2)) - 0.5)
            if self.radius <= 1:
                self.radius = 1
            ...
            #画出神经网络的规划图
            if not i % 1000:
                plot_network(citys, network, name='GSOMgif/{:05d}.png'.format(i))
        plot_network(citys, network, name='GSOMgif/final.png')
